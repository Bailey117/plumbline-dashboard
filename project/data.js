// Shared sample data for the LME / Battery Recycling dashboard.
// Plausible AU market figures, May 2026 baseline.
// LME Pb cash ≈ USD 2,055/t · AUD/USD ≈ 0.655 · AIP terminal gate ≈ AUD 2.05/L

(function (global) {
  // ── Market data ────────────────────────────────────────────────────────────
  const market = {
    lme_pb_usd: 2055.50,        // USD / tonne, LME Lead cash settlement
    lme_pb_usd_prev: 2031.00,
    aud_usd: 0.6548,            // RBA AUD/USD
    aud_usd_prev: 0.6612,
    diesel_gate_aud: 2.047,     // AIP terminal gate price, AUD/L (ex GST)
    diesel_gate_aud_prev: 2.018,
    fuel_levy_aud: 0.501,       // c/L excise (illustrative combined)
  ...
  // ── 90-day history (one point per day, oldest → newest) ───────────────────
  // Procedural but deterministic so charts look real.
  ...
  // ── Freight zones (tiered, diesel-indexed) ────────────────────────────────
  // Base $/t at diesel = 1.80 AUD/L; effective = base × (1 + (diesel-1.80)/1.80 × 0.42)
  const freight_zones = [
  ...
  function freightAt(zoneCode, dieselPrice) {
  ...
  // ── Suppliers (50 realistic AU battery recyclers / auto wreckers) ─────────
  // pct_lme = supplier's lead price as % of LME AUD spot.
  const supplierSeed = [
  ...
  const suppliers = supplierSeed.map(([name, state, zone, pct, daysSince, monthly], i) => {
    // Per-supplier 90-day price series in AUD/t (= pct% of LME AUD)
    const series = history.lme_pb_aud.map(p => +(p * pct / 100).toFixed(2));
    const freightSeries = history.diesel.map(d => freightAt(zone, d));
    // Monthly volume jitter
    ...
    return {
      ...
      ytd_spend_aud: 0, // computed below
  ...
  // ── Sites: each supplier has 1-3 pickup sites, zone is PER-SITE ──────────
  // Most suppliers (35) have one site = the primary. Some have 2-3 sites,
  // sometimes across states.
  const MULTI_SITE = {
    // index in supplierSeed → array of [site_name_suffix, state, zone, monthly_share]
  ...
  // ── Daily volume series (last 90 days, tonnes/day, all suppliers) ──────
  const volume_daily = history.days.map((d, i) => {
    const s = (i * 73) % 233;
    const base = suppliers.reduce((a, x) => a + x.avg_monthly_t, 0) / 30;
    return +(base * (0.7 + (s / 233) * 0.6)).toFixed(1);
  });
  const spend_daily = volume_daily.map((v, i) => Math.round(v * (history.lme_pb_aud[i] * 0.74 + 60)));
  ...
  const states = STATES.map(code => {
    ...
    return {
      code,
      name: { NSW:"New South Wales", VIC:"Victoria", QLD:"Queensland", WA:"Western Australia",
              SA:"South Australia", TAS:"Tasmania", NT:"Northern Territory", ACT:"Aust. Capital Terr."}[code],
  ...
  global.DASH_DATA = { market, history, freight_zones, freightAt, suppliers, states, fmt, STATES, alerts, volume_daily, spend_daily, months };
})(window);
